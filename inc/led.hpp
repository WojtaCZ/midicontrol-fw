...
namespace LED{
    ...
    class Peripheral{
        private:
            Strip strip = Strip::BACK;
            LedID led;

        public:
            Peripheral(LedID led) : led(led){};
            void setColor(Color color);
    };

    void init(void);
    void set_color(uint8_t strip, uint32_t LEDnumber, uint8_t RED, uint8_t GREEN, uint8_t BLUE);
    void set_led_color(uint8_t strip, uint32_t LEDnumber, uint8_t RED, uint8_t GREEN, uint8_t BLUE);
    ...
    void dev_process_pending_status(void);
